...
function Library:open()
    ...
    local containerSize, shadowSize
...
UserInputService.InputBegan:Connect(function(input, process)
    ...
    local chosenKey = Library.Flags.fly_key or "H"
  ...
  function Tab:create_tab()
        ...
        if self=='Auto' then
        Logo.Image = "rbxassetid://7734051454" end
        if self=='Main' then
        Logo.Image = "rbxassetid://17290697757" end
        if self=='Teleports' then
        Logo.Image = "rbxassetid://7734020989" end  
        if self=='Settings & Config' then
        Logo.Image = "rbxassetid://8997386997" end      
        ...
        function Module:create_toggle()
            ...
            if UserInputService.TouchEnabled then
                            Checkbox.Position = UDim2.new(0.93, 0, 0.5, 0)
                        Checkbox.Size = UDim2.new(0, 20, 0, 20)     
            else
                        Checkbox.Position = UDim2.new(0.95, 0, 0.5, 0)
                        Checkbox.Size = UDim2.new(0, 20, 0, 20)     
            end
    ...
    TextBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        TextLabel_2.Size = UDim2.new(1, -4, 1, -4)
        ...
        TextLabel_2.Text = Library.Flags[self.flag]

        local function updateKeybindSize()
            local textWidth = TextLabel_2.TextBounds.X
            local PADDING_X = 15
            Box.Size = UDim2.new(0, textWidth + PADDING_X, 0, Box.Size.Y.Offset)
        end

        TextLabel_2:GetPropertyChangedSignal("Text"):Connect(updateKeybindSize)
        updateKeybindSize()
    ...
    if keybindListener then
        keybindListener:Disconnect()
        keybindListener = nil
    end

    local a = UserInputService.InputBegan:Wait()
    if a.UserInputType == Enum.UserInputType.Keyboard and a.KeyCode ~= Enum.KeyCode.Unknown then
        Library.Flags[self.flag] = a.KeyCode.Name
        Library.save_flags()
        TextLabel_2.Text = a.KeyCode.Name
    end

    game:GetService("RunService").RenderStepped:Wait()
    isSettingKeybind = false 

    keybindListener = UserInputService.InputBegan:Connect(function(current, pressed)
        if pressed then return end
        if current.UserInputType ~= Enum.UserInputType.Keyboard then return end
    ...
    local UIGradient = Instance.new("UIGradient")
    UIGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 255, 120)), 
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 230, 80)) 
    }
...
function Module:create_multidropdown()
    ...
    Dropdown.selectionChanged = false
    Dropdown.oldSelectedOptions = {}
    ...
    function Dropdown:open()
        ...
        self.oldSelectedOptions = copyTable(self.selectedOptions)
        self.selectionChanged = false

        TweenService:Create(Options, TweenInfo.new(0.4), {
            Size = UDim2.new(1, 0, 0, list_size + 10)
        }):Play()

        TweenService:Create(dropdown, TweenInfo.new(0.4), {
            Size = UDim2.new(BASE_WIDTH, 0, 0, 40 + list_size + 10)
        }):Play()
    ...
    function Dropdown:close()
        ...
        if not self.selectionChanged then
       
            self.selectedOptions = copyTable(self.oldSelectedOptions)
        end

        if next(self.selectedOptions) then
        ...
        TweenService:Create(dropdown, TweenInfo.new(0.4), {
            Size = UDim2.new(BASE_WIDTH, 0, 0, CLOSED_HEIGHT)
        }):Play()
    ...
    function Dropdown:update()
        ...
        for _, value in ipairs(self.options) do
            ...
            if type(self.selectedOptions) ~= "table" then
            ...
            new_option.MouseButton1Click:Connect(function()
                if table.find(self.selectedOptions, value) then
                    for i, v in ipairs(self.selectedOptions) do
                        if v == value then
...
function Module:create_dropdown()
    ...
    Dropdown.selectedOption = nil -- текущий выбор
    ...
    function Dropdown:update()
        ...
        for _, value in ipairs(self.options) do
            ...
            -- Подсвечиваем только текущий выбранный option
            if value == self.selectedOption then
                new_option.TextTransparency = 0
            else
                new_option.TextTransparency = 0.5
            end
    ...
    -- Инициализируем с пустым выбором
